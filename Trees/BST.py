...
class BST:
        ...
        while len(resur1) != 0:
            for x in resur1:
                ...
                if x.LeftChild is not None:
                    resur2.append(x.LeftChild)
                if x.RightChild is not None:
                    resur2.append(x.RightChild)
            resur1 = resur2
            resur2 = []
        return res

    def DeepAllNodes(self,n):
        res = []
        if n == 0:
            res = self.__inorder(self.Root)
        elif n == 1:
            res = self.__pastorder(self.Root)
        elif n == 2:
            res = self.__preorder(self.Root)
        return res
    ...
    def __pastorder(self,Node):
        ...
        if Node.RightChild is not None and Node.LeftChild is not None:
            ...
            res =  self.__pastorder(Node.LeftChild) + [Node]
