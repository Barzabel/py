            ...
            for x in range(len(g.m_adjacency)):
                if sum(g.m_adjacency[x])%2 ==0:
                    for y in range(len(g.m_adjacency[x])):
                        if g.m_adjacency[x][y]==1 and sum([ c[y] for c in g.m_adjacency])%2==0:
...
class EvenTrees(unittest.TestCase):

    def setUp(self):
        a = SimpleTreeNode(0, None)
        self.Tree = SimpleTree(a)
    ...
    def test_EvenTrees1(self):
        res = self.Tree.EvenTrees()
        for x in range(0,len(res)-1):
            self.assertTrue(res[x]!=res[x+1])

    def test_EvenTrees2(self):
        res = self.Tree.EvenTrees()
        for x in range(0,len(res)):
            self.assertTrue(type(res[x]==type(SimpleTreeNode(1,None))))
